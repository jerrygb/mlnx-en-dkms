...
#checkout libfc/libfcoe headerfiles
if [ ! -e ofed_scripts/libfc_header_checkout_files ]; then
	ex git checkout ${branch} ofed_scripts/libfc_header_checkout_files
fi

xex xargs -rt git ls-tree -r --name-only ${branch} <ofed_scripts/libfc_header_checkout_files |
	xex xargs -rt git checkout ${branch}
